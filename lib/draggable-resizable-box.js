...
export default React.createClass({
  ...
  onResize (event) {
    ...
    this.setState(Object.assign({}, this.getClientCoordinates(event), position, dimensions), () => {
      this.props.onChange({
        top: position.top,
        left: position.left
      }, dimensions)
  ...
  controlsMoveBox (evt) {
    let movedX = evt.target.name === 'x' ? evt.target.value - this.state.left : this.state.left
    let movedY = evt.target.name === 'y' ? evt.target.value - this.state.top : this.state.top
